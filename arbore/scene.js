...
export function createScene(container, callbacks = {}) {
  ...
  let opts = { glow: 1, curve: 1, fileSize: 1, labelDepth: 1, autoRotate: false };
  ...
  const discTex = discTexture(), glowTex = glowTexture();
  ...
  function refresh() {
    ...
    labels.forEach((l) => {
      ...
      const depthOn = d.depth <= opts.labelDepth || (isolated && topNode === isolated && d.depth <= opts.labelDepth + 2);
  ...
  function setOption(k, v) {
    ...
    if (k === 'curve') { branchSegGroups.forEach(({ line }) => sceneRoot.remove(line)); buildBranches(); }
  ...
  // ===== build + start =================================================
  buildBranches(); buildPoints(); buildGlows(); buildLabels();
  applyColors(); refresh();
  ...
  return {
    ...
    getLegend: () => branches.map((b) => ({ name: b.name, color: '#' + theme.palette[b.cidx].toString(16).padStart(6, '0'), node: b.node, count: b.count })),
