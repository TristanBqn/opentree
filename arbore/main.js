  ...
  onCounts({ match, total, isolated }) {
    $("#count").textContent = isolated
      ...
      : match < total
        ...
        : `${total} fichiers · ${scene ? scene.getStats().dirs : ""} dossiers`;
...
renderLegend();
$("#count").textContent =
  `${scene.getStats().files} fichiers · ${scene.getStats().dirs} dossiers`;
...
function applyTheme(key, save = true) {
  ...
  document
    ...
    .forEach((b) => b.classList.toggle("on", b.dataset.theme === key));
  document.body.dataset.theme = key;
  scene.setTheme(key);
  renderLegend();
  if (save)
    try {
      localStorage.setItem("arbore.theme", key);
    } catch (e) {}
}
...
applyTheme(
  (() => {
    try {
      return localStorage.getItem("arbore.theme");
    } catch (e) {
      return null;
    }
  })() || "parchment",
