...
export function createScene(container, callbacks = {}) {
  ...
  let opts = { glow: 1, curve: 1, fileSize: 1, labelDepth: 1, autoRotate: false, pulseSpeed: 1, pulseIntensity: 1, plates: true };
  ...
  function buildIslandChrome() {
    islands.forEach((isl) => {
      ...
      el.innerHTML = `
        <div class="ip-name"><span class="ip-dot st-${isl.status}"></span>${isl.name}</div>
        <div class="ip-meta">${isl.statusTxt} · ${isl.stats}</div>
        <div class="ip-ports">${isl.ports}</div>`;
      el.addEventListener('pointerdown', (e) => { e.stopPropagation(); callbacks.onIsland?.(isl); });
  ...
  function buildLabels() {
    ...
    dirs.forEach((d) => {
      ...
      el.addEventListener('pointerdown', (e) => { e.stopPropagation(); callbacks.onLabel?.(d); });
      el.addEventListener('pointerenter', (e) => callbacks.onHover?.(d, e.clientX, e.clientY));
  ...
  function refresh() {
    ...
    labels.forEach((l) => {
      ...
      // les dossiers des containers ne sont étiquetés qu'en isolation (la plaque suffit)
      const isCont = d.iid !== 'host';
      const baseDepth = isCont ? opts.labelDepth - 1 : opts.labelDepth;
      const depthOn = d.depth <= baseDepth || (isolated && topNode === isolated && d.depth <= opts.labelDepth + 2);
  ...
  function animate() {
    ...
    labels.forEach((l) => {
      ...
      const o = THREE.MathUtils.clamp(1 - (d - 18 * sizeScale) / (90 * sizeScale), 0.3, 1);
      l.el.style.opacity = (l.el.classList.contains('dim') ? o * 0.25 : o).toFixed(2);
    });
