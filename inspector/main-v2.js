...
function sparkSVG(usage, w = 132, h = 30) {
  ...
  const line = usage.map((v, i) => `${px(i).toFixed(1)},${py(v).toFixed(1)}`).join(' ');
  const area = `2,${h - 3} ${line} ${w - 2},${h - 3}`;
  const lastI = 29, lastV = usage[29];
...
const callbacks = {
  ...
  onPick(file, x, y) { openFileBubble(file, x, y); },
  onEmpty() { tooltip.classList.remove('show'); },
  onLabel(node, x, y) { openNodeBubble(node, x, y); },
  onIsland(isl) { openIsland(isl); },
  onCounts({ match, total, isolated }) {
    const st = scene ? scene.getStats() : null;
    $('#count').textContent = isolated
      ? `${isolated} · ${match} fichiers`
      : (match < total ? `${match} / ${total} fichiers` : (st ? `${st.containers} conteneurs · ${total} fichiers · ${st.dirs} dossiers` : `${total} fichiers`));
...
function renderLegend() {
  ...
  islands.filter((i) => i.kind === 'container').forEach((isl) => {
  ...
  legend.filter((l) => l.islandId === 'host').forEach((l) => {
    const b = document.createElement('button');
    b.className = 'legend-chip';
...
$('#count').textContent = `${st0.containers} conteneurs · ${st0.files} fichiers · ${st0.dirs} dossiers`;
...
// ---- settings popover ---------------------------------------------------------
$('#gear').addEventListener('click', () => { $('#arch-panel').classList.remove('open'); $('#settings').classList.toggle('open'); });
$('#arch-btn').addEventListener('click', () => { $('#settings').classList.remove('open'); $('#arch-panel').classList.toggle('open'); });
const PCT = (v) => Math.round(v * 100) + '%';
const optDefs = {
  glow: PCT, curve: PCT, fileSize: PCT,
};
...
$('#opt-plates').addEventListener('change', (e) => scene.setOption('plates', e.target.checked));

// ---- étiquettes : on/off + niveau d'affichage (dernier cran = Automatique) ----
$('#opt-labels').addEventListener('change', (e) => scene.setOption('labelsOn', e.target.checked));
...
function applyLabelLevel() {
  ...
  if (v >= autoPos) { scene.setOption('labelAuto', true); labelVal.textContent = 'Automatique'; }
  else { scene.setOption('labelAuto', false); scene.setOption('labelDepth', v); labelVal.textContent = 'Niveau ' + v; }
}
...
function openDetail(file) {
  ...
  const locRow = isl && isl.kind === 'container'
  ...
  const perDay = file.uses ? (file.uses / 30).toFixed(1).replace('.', ',') : '0';
...
function metricTile(chunk) {
  ...
  if (m) return `<div class="ic-tile"><span class="t-k">${m[1]}</span><span class="t-v">${m[2] || '—'}</span></div>`;
  return `<div class="ic-tile"><span class="t-v">${chunk}</span></div>`;
}
function openIsland(isl) {
  activeIsland = isl;
  const leg = scene.getLegend().find((l) => l.islandId === isl.id && l.node === isl.root);
  ...
  const tiles = isl.stats.split('·').map((s) => s.trim()).filter(Boolean).map(metricTile).join('');
  const chips = isl.ports.split('·').map((s) => s.trim()).filter(Boolean)
    .map((c) => `<span class="ic-chip">${c}</span>`).join('');
  ...
  islandCard.querySelector('#ic-isolate').addEventListener('click', () => {
    const b = scene.getLegend().find((l) => l.islandId === isl.id && l.node === isl.root);
    if (b) scene.setIsolated(b.node); else scene.flyTo(isl.root);
    closeIsland();
  });
  islandCard.querySelector('#ic-center').addEventListener('click', () => scene.flyTo(isl.root));
}
...
function markActivePlate(isl) {
  document.querySelectorAll('.island-plate').forEach((el) => {
    el.classList.toggle('active', !!isl && el.querySelector('.ip-title')?.textContent === isl.name);
  });
}
...
function countUnder(node) {
  ...
  (function w(n) {
    (n.children || []).forEach((c) => {
      if (c.type === 'dir') { dirs++; w(c); } else files++;
    });
  })(node);
  return { dirs, files };
}
function branchColorOf(node) {
  ...
  return (top && scene.getLegend().find((l) => l.node === top)?.color) || '#8a8275';
}
...
function openNodeBubble(node, x, y) {
  ...
  nodeBubble.querySelector('#nb-iso').addEventListener('click', () => { isolateNode(node); closeBubble(); });
  placeBubble(x, y);
  enableBubbleDrag();
}
...
function closeBubble() { nodeBubble.classList.remove('show'); }
...
function enableBubbleDrag() {
  ...
  head.addEventListener('pointerdown', (e) => {
    ...
    const move = (ev) => {
      const nx = Math.max(6, Math.min(ev.clientX - ox, innerWidth - r.width - 6));
      const ny = Math.max(6, Math.min(ev.clientY - oy, innerHeight - r.height - 6));
...
addEventListener('keydown', (e) => {
  if (e.key === 'Escape') { tooltip.classList.remove('show'); closeBubble(); closeIsland(); scene.resetView(); detail.classList.remove('show'); }
