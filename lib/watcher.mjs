...
export function isIgnoredPath(rel, absRoot, eventsPath) {
  if (rel.includes("node_modules") || rel.includes(".git")) return true;
  // ne jamais réagir à notre propre journal (sinon boucle si eventsPath est
  // sous une racine surveillée, ex. .openclaw)
  const abs = resolve(absRoot, rel);
  return abs === resolve(eventsPath) || abs === resolve(eventsPath + ".tmp");
}
...
function watchRoot(absRoot, rootName, eventsPath) {
  try {
    const watcher = watch(absRoot, { recursive: true }, (_evt, filename) => {
      ...
      if (isIgnoredPath(rel, absRoot, eventsPath)) return;
